...
The A/B harness creates a unique `--test-run-id`, stores harness artifacts under `artifacts/ab_tests/<test-run-id>/`, and writes `result.json` with build identity, timing, cleanup evidence, `result`, `layer`, `stage`, and `reason`. Its explicit checkpoints are lobby creation, lobby membership, peer creation, assignment to Godot's `MultiplayerApi`, native peer handshake, Godot connection signals, GameFactory lifecycle, and replication. Per-process `game.jsonl` and `engine.log`, the host-collected session, the build manifest, VM parity result, and a focused native-term extract are retained for each attempt. Its test-only `steam_basic` scenario is activated only by `--test-scenario=steam_basic`; normal and manual probe runs do not execute it.
